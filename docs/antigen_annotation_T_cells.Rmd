...
Previously generated a dataset of TCR beta chain (TRB) clonotypes of known antigen/epitope specificity that were collected from two resources; SARS-CoV-2 from [immuneCODE]() and various antigens from [VDJdb](). These resources were parsed and wrangled into a format that will merge with the 10x T cell data in the [generating_Ag_reference_for_TRB.R]() script which is described [here]().
 
To annotate the clonotypes with antigen specificity we will rely of exact matches between the clonotype labels between the datasets; so using V + J + CDR3AA sequence. It is not uncommon to used just the CDR3 or junction AA sequence as an alternative 'complete' clonotype matching.
...
There are two datasets to load. The seurat data and the antigen annotated TRB clonotypes. In the first instance, to save on memory, loading the seurat metadata + UMAP coords data only from the tab-delimited file rather than the complete Suerat objects.
```{r loading_datasets}
...
## annotated TCRs
trb.ags.data <- read_tsv("~/Documents/GitHub/scRNA-seq_VDJ/tcr_ag_refs/trb_antigens.tsv")
```

TODO - add URL alternatives for loading from dropbox/github

## Annotating the TRB clonotypes

Given that the antigen specific TCRs are bias towards TRB chains, annotation is using TRB only. Currently, the TCR clonotype for the single cell data is formatted as a paired clonotype label that combines the TRB:TRA. To join the single cell data with the antigen data, first extract the TRB clonotype from the paired_clonotype and then join with the antigen data using a left_join. Note that the antigen data was already collasped so that each unique TRB is listed only once with the epitopes/antigens/source appended in comma delimited lists. This is necessary to ensure a one-to-one or many-to-one relationship when doing the left_join. If there are multiple matches in the antigen data (ie, one-to-many or many-to-many) the entries in the seurat.data will become duplicated for each match and the data will no longer be one entry for cell barcode.
