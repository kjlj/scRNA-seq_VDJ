...
The VDJ is integrated to the Seurat object using metadata slots. This permits the VDJ features to be used in plotting and other Seurat functions such as finding gene markers. 
...
Requires [Seurat](https://satijalab.org/seurat/) to interact with the Seurat object generated from processing of the gene expression data. Assumes this has been saved as an RDS. Once again will be be using the [tidyverse packages](https://www.tidyverse.org/) for data manipulation, [ggpubr](https://rpkgs.datanovia.com/ggpubr/) for plot layouts and also adding stats to panels, [ggsci](https://cran.r-project.org/web/packages/ggsci/vignettes/ggsci.html) for colour palettes and rstatix for generating ggpubr/ggplot2 compatible data for showing stats on panels.
...
Loading the [VDJ data](https://kjlj.github.io/scRNA-seq_VDJ/docs/joining_cellranger_igblast.html) for each cell that is the merging of the cellranger and IgBLAST outputs. For the B cells this includes the [clone calling](https://kjlj.github.io/scRNA-seq_VDJ/docs/building_b_cell_clones.html).
...
Up to now have treated the B and T VDJ data within the same dataset independently but they are now going to be united with the scRNA-seq data. This means that for each dataset, each cell barcode can only have a single entry - either B or T VDJ data. There shouldn't be cells with both B and T VDJ for the same cell barcode, but doublets (which haven't been filtered from the VDJ) can result in this artifact. Cell barcodes that have both B and T VDJ data need to be identified and removed:
...
There may also be cell barcodes for which there is VDJ data but the cell barcode was removed from GEX during filtering. To harmonise the cell barcodes between the VDJ and GEX, and drop VDJ cell barcodes that don't have data in the GEX get a list of the cell barcodes for the PBMC and tumour samples from the Seurat objects. This can be obtained by getting the rownames from the meta.data slot in the objects:
...
For the T cells, removing the cells that don't have the barcodes, and then adding paired clonotype labels, clone size and whether or not the cell is part of a clonal expansion:
```{r add_t_cell_data}
...
Ranking for the clones/clonotypes can also be added:
```{r adding_cln_ranks}
...
The B and T cell data can now be formatted to merge with the Seurat objects for the scRNA-seq analysis from the GEX. The join will happen via the barcode and here we are selecting the subset of columns that want to add into the Seurat metadata. This list can be expanded/contracted as needed:
...
#remove the data now done with it
rm(list = c("tumour.metadata", "b.metadata", "t.metadata"))
```

## Plotting using VDJ metadata

Now that the VDJ features are in the meta.data slot for seurat objects they can be used for plotting. Start with highlighting which cells have B or T VDJ data:
```{r seurat_plots_1}
...
Plotting which cells are using different isotype subclasses or TRBC genes:
```{r seurat_plots_2}
...
Not limited to UMAPS, can also use the VDJ meta data for other plots types. For example, can look at gene expression of CD3 for each seurat cluster for T cell clonotypes that are either expanded or singleton:
```{r seurat_plots_7}
...
Or, showing relative gene expression for on a feature plot:
```{r seurat_plots_8}
...
Plotting within Seurat can lack some flexibility, so rather than importing data to the Seurat object, data can also be exported from Seurat to use the tidyverse functions. Data is exported from the meta.data and the UMAP coordinates slots.
...
Now have greater control over the plotting & the format of the plots and we have freed up a lot of memory by unloading the seurat objects.
```{r ggplot_umap}
...
We can build more complex panels using ggplot + ggpubr. For example, visualing the SHM of the B cells on UMAPs and displaying side-by-side with a quantification of the SHM% for clones for each isotype with statistics:
```{r b_cell_shm_panel}
...
Exploring the extend of clone/clonotype expansion in the different datasets:
```{r clonal_exp_panel}
...
One challenge when using data point fill/colour to encode a VDJ feature like SHM is that it becomes hard to track the clusters on the UMAP. Hull plots can be used to automate the generation of outlines for the clusters. To use hull plots need to laod the ggforce package:
```{r load_ggforce}
