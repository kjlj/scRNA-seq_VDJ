...
The VDJ is integrated to the Seurat object using metadata slot. This permits the VDJ features to be used in plotting and other Seurat functions such as finding gene markers. 
...
Cell barcodes that have both B and T VDJ data need to be identified and removed:
...
There may also be cell barcodes for which there is VDJ data but the cell was removed from GEX during filtering. To harmonise the cell barcodes between the VDJ and GEX, and drop VDJ cell barcodes that don't have data in the GEX datasets. 

To do this we need a list of the cell barcodes for the PBMC and tumour samples from the Seurat objects. This can be obtained by getting the rownames from the meta.data slot in the objects:
...
For the T cells, removing the cells that don't have the barcodes in GEX, and then adding paired clonotype labels, clone size and whether or not the cell is part of a clonal expansion:
```{r add_t_cell_data}
...
Ranks for the expanded clones/clonotypes can also be added (which #1 being the largest clone/clonotype in the dataset):
```{r adding_cln_ranks}
...
The B and T cell data can now be formatted to add to the Seurat objects for the scRNA-seq analysis from the GEX. The join will happen via the barcode and here we are selecting the subset of columns that want to add into the Seurat meta.data. This list can be amended as needed:
