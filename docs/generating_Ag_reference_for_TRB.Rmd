---
title: "Generating TCR specificity reference"
...
The epitope specificity of T cells may be inferred by matching TCRs to a reference database of clonotypes of known specificity. TCRs of known specificity will be collected from two sources and formatted for consistency with the clonotype labels used in the 10x data. 
...
The database files are distributed as a zip archive, so uncompress it (after which the .zip can be removed):
...
First make a directory, change to it and then use cURL to download the file:
...
As for vdjdb, the file comes are a zip archive that needs to be 'unzipped':
...
Load the downloaded files for immuneCODE into R. There are TCRs (only TRB) from three different assay; minigene, class I peptides and class II peptides. All three will be used:
...
5. remove any entries where the V is NA, these are extra entries created from clonotypes that didn't have second V genes
...
A quick summary of the clonotypes and their antigens:
```{r antigen_summary}
