...
This workflow uses the [IMGT CDR3 definition](https://www.imgt.org/IMGTScientificChart/Numbering/IMGTIGVLsuperfamily.html) which excludes the 104-Cys and 118-Phe/Tyr. There are other CDR3 definitions that incorporate these residues. The equivalent 'CDR3' can be obtained fro, the junction and junction_aa columns reported by `IgBLAST` which will include the anchor residues.

As the B cell chains undergo somatic hypermutation (SHM) need to use a method for defining clonal groups that permits grouping of similar but not exact sequences.  Hierarchical clustering of the Ig sequences CDR3 sequence is one approach used to identify groups of cells that carry sequences that are derived from the same original progenitor B cell. These cells will carry heavy and light chains with similar nucleotide sequences that have diverged due to the accumulation of point mutations. 

Clustering must be undertaken for each individual as a clonal lineage can only exist within a single individual. Similar Igs from different individuals are 'convergent' antibodies, that may have arisen from response to the same antigen, rather than 'divergent' members of the same clone. The same type of clustering can be used to explore convergence (also known as public clones) in antibody responses but this is in addition to building clonal lineages within an individual's immune response.

In the example datasets the pbmc and tumour samples are from different individuals so clones will be built separately. If 10x datasets includes multiple samples that require de-multiplexing via hashing antibodies or SNPs that the B cell V(D)J data must be subset before building clones. 

An excellent resource for post-processing of 10x data including building clonal relationships can be found at on the   [Immcantation frameword site](https://immcantation.readthedocs.io/en/stable/tutorials/10x_tutorial.html). It provides an approach can be undertaken in R, but can also export data and use alternative tools such as [cd-hit](https://github.com/weizhongli/cdhit) but this requires some additional coding to wrangle the cd-hit output into a format that can be joined to the `Cell Ranger` and `IgBLAST` cell barcode level information. 

The RScript for this workflow available on [github](https://raw.githubusercontent.com/kjlj/scRNA-seq_VDJ/main/RScripts/building_b_cell_clones.R) in the [workshop respository](https://github.com/kjlj/scRNA-seq_VDJ).
...
This workflow uses the Ig dataset that was output from [previous workflow](https://kjlj.github.io/scRNA-seq_VDJ/docs/joining_cellranger_igblast.html) or the [joining_cellranger_igblast.R](https://raw.githubusercontent.com/kjlj/scRNA-seq_VDJ/main/RScripts/joining_cellranger_igblast.R) script. The tab-delimited output file ([pbmc-tumour_Ig_cellranger_igblast_per-barcode.tsv](https://raw.githubusercontent.com/kjlj/scRNA-seq_VDJ/main/data/pbmc-tumour_Ig_cellranger_igblast_per-barcode.tsv)) that contains the Ig VDJ data for the PBMC and tumour datasets is hosted on the [github repository](https://github.com/kjlj/scRNA-seq_VDJ).

The data will be loaded from the githib repo, but can alternatively be loaded locally by changing to a file path rather than a URL within the `read_tsv` function:
```{r loading_datasets}
...
Before performing the hierarchical clustering the clustering threshold needs to be defined. This is done by comparing the pairwise [hamming distance](https://en.wikipedia.org/wiki/Hamming_distance) between sequences in a dataset to find each sequences nearest distance. This should result in a bimodal distribution that defines 'clones' (sequences very near to each other) and 'unrelated' (sequences more distant from each other). 

For this, we can use the IGH data. 

The `SHazaM` package expects `IgBLAST` column names in their orignal format so we need to extract those columns and rename them to remove the `_iglbast_igh` suffix that we appended previously:
```{r igh_shazam_format}
...
Plotting the distances to look for the valley between the bimodal distribution that should provide a threshold between 'clonal' and 'unrelated' sequences:
...
Now, repeat the process for the IGH from the tumour dataset:
...
To run the `SCOPer` clone building in 'single-cell' mode need to reformat the tibble to have the igh and igkl for each cell barcode as separate rows:
...
Extract the clone IDs so that they can be added to the original tibble that summarises the `IgBLAST` and `Cell Ranger` for each cell barcode:
```{r extract_cln_ids}
