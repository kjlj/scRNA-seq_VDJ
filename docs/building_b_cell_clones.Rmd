...
An excellent resource for post-processing of 10x data including building clonal relationships can be found at on the   [Immcantation framework site](https://immcantation.readthedocs.io/en/stable/tutorials/10x_tutorial.html). It provides an approach can be undertaken in R, but can also export data and use alternative tools such as [cd-hit](https://github.com/weizhongli/cdhit) but this requires some additional coding to wrangle the cd-hit output into a format that can be joined to the `Cell Ranger` and `IgBLAST` cell barcode level information. 
